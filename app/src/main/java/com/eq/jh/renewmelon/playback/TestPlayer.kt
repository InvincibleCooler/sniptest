package com.eq.jh.renewmelon.playback
...
class TestPlayer(private val context: Context) {
    companion object {
        private const val TAG = "TestPlayer"
    }

    interface TestPlayerCallback {
        fun onCompletion()
        fun onPlaybackStateChanged(playWhenReady: Boolean, state: Int)
        fun onError(error: String)
    }

    private var callback: TestPlayerCallback? = null

    fun setCallback(callback: TestPlayerCallback) {
    ...
    private fun buildDataSourceFactory() =
        DefaultDataSourceFactory(context, Util.getUserAgent(context, "Test"))
